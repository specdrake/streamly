...
mkAccumM step initial = Fold (\s a -> Partial <$> step s a) (Partial <$> initial)
...
{-# INLINE mkFoldM #-}
mkFoldM ::
       Functor m => (s -> a -> m (Step s b)) -> m s -> (s -> m b) -> Fold m a b
mkFoldM step initial = Fold step (Partial <$> initial)
...
sequence (Fold step initial extract) = Fold step' initial1 extract'
    ...
    -- XXX Reduce code duplication after writing proper benchmarks
    initial1 = do
        res <- initial
        case res of
...
transform (Pipe pstep1 pstep2 pinitial) (Fold fstep finitial fextract) =
    ...
    initial = do
        ...
        return
            $ case res of
                  Partial fs -> Partial $ Tuple' pinitial fs
                  Done fb -> Done fb
...
_Fold1 step = Fold step_ (return (Partial Nothing')) (return . toMaybe)
...
drainBy f =
    Fold (const (fmap FL.Partial . void . f)) (return (Partial ())) return
...
genericLength =
    Fold (\n _ -> return $ Partial $ n + 1) (return (Partial 0)) return
...
sum = Fold (\x a -> return $ Partial $ x + a) (return (Partial 0)) return
...
product = Fold step (return (Partial 1)) return
...
mean = Fold step (return (Partial begin)) (return . done)
...
variance = Fold step (return (Partial begin)) (return . done)
...
sconcat i = Fold (\x a -> return $ Partial $ x <> a) (return (Partial i)) return
...
genericIndex i = Fold step (return (Partial 0)) (const (return Nothing))
...
find predicate = Fold step (return (Partial ())) (const (return Nothing))
...
lookup a0 = Fold step (return (Partial ())) (const (return Nothing))
...
findIndex predicate = Fold step (return (Partial 0)) (const (return Nothing))
...
distribute_ fs = Fold step initial extract
    ...
    initial = Partial <$> Prelude.mapM initialize fs
        ...
        case resi of
            Partial i1 -> do
                ...
                case res of
                    Partial i2 -> do
                        rst <- go ss a
                        let fld = Fold s (return (Partial i2)) d
                        return $ fld : rst
                    Done () -> go ss a
            Done () -> go ss a
    ...
    go1 [] = return ()
    go1 ((Fold _ si d):ss) = do
      res <- si
      case res of
          Partial i -> d i >> go1 ss
          Done _ -> go1 ss
...
demuxWith f kv = Fold step initial extract
    ...
    initial = do
        let func fld@(Fold _ si _) = do
            ...
            isLeft (Left' _) = True
            isLeft _ = False
        emap <- Prelude.mapM func kv
        let mp = Map.map fromRight $ Map.filter isRight emap
            bmp = Map.map fromLeft $ Map.filter isLeft emap
        return
            $ if Map.size mp > 0
              then Partial $ Tuple' bmp mp
              else Done bmp

    step (Tuple' bmp mp) a = do
        let (k, a') = f a
        case Map.lookup k mp of
            Nothing -> return $ Partial $ Tuple' bmp mp
            Just (Fold stp ini dn) -> do
                -- XXX Here ini can never be Done
                sini <- ini
                case sini of
                    Partial st -> do
                        !res <- stp st a'
                        return
                            $ case res of
                                  Partial sres ->
                                      let ini1 = return $ Partial sres
                                          fld = Fold stp ini1 dn
                    ...
                    -- XXX Maybe we should define this with "MonadError m =>"
                    Done _ -> error "Illegal state"

    extract (Tuple' bmp mp) = do
        let extF (Fold _ si d) = do
                res <- si
                case res of
                    Partial s -> d s
                    Done b -> return b
        mpe <- Prelude.mapM extF mp
...
demuxWithDefault_ f kv (Fold dstep dinitial dextract) =
    ...
    initial = do
        ...
        let mp = Map.map fromRight $ Map.filter isRight emap
            bst =
                Set.fromList
                    $ Map.keys $ Map.map fromLeft $ Map.filter isLeft emap
        res <- dinitial
        return
            $ if Map.size mp > 0
              then Partial
                       $ case res of
                             Partial ini -> DemuxingWithDefault ini bst mp
                             Done _ -> DemuxingOnlyMap mp
              else Done ()

    -- XXX Some code can probably be abstracted
    step (DemuxingWithDefault dacc bst mp) a = do
        let (k, a1) = f a
        case Map.lookup k mp of
            Nothing ->
                if k `Set.member` bst
                            ...
                            $ case res of
                                  Partial sres ->
                                      let ini1 = return $ Partial sres
                                          ...
                                          mp1 = Map.insert k fld mp
                                       in Partial
                                              $ DemuxingWithDefault dacc bst mp1
                                  Done _ -> do
                                      let mp1 = Map.delete k mp
                                          bst1 = Set.insert k bst
                                          next =
                                              DemuxingWithDefault dacc bst1 mp1
                                      if Map.size mp1 == 0
                                      then Done ()
                                      else Partial next
                    -- XXX Maybe we should define this with "MonadError m =>"
                            ...
                            $ case res of
                                  Partial sres ->
                                      let ini1 = return $ Partial sres
                                          fld = Fold stp ini1 dn
                                          mp1 = Map.insert k fld mp
                                       in Partial $ DemuxingOnlyMap mp1
                                  Done _ -> do
                                      let mp1 = Map.delete k mp
                                      if Map.size mp1 == 0
                                      then Done ()
                                      else Partial $ DemuxingOnlyMap mp1
                    -- XXX Maybe we should define this with "MonadError m =>"
                    Done _ -> error "Illegal state"
    ...
    extract st = do
        let extF (Fold _ si d) = do
                ...
                case res of
                    ...
                    Done b -> return b
        case st of
            DemuxingWithDefault dacc _ mp -> do
                void $ dextract dacc
                Prelude.mapM_ extF mp
            DemuxingOnlyMap mp -> Prelude.mapM_ extF mp
...
demuxWith_ f kv = Fold step initial extract
    ...
    initial = do
        let func fld@(Fold _ si _) = do
            ...
            fromMaybe (Just' x) = x
            fromMaybe _ = undefined

            isJust' (Just' _) = True
            isJust' _ = False
        emap <- Prelude.mapM func kv
        let mp = Map.map fromMaybe $ Map.filter isJust' emap
            sz = Map.size mp
        return
            $ if sz > 0
              then Partial $ Tuple' sz mp
              else Done ()

    step (Tuple' n mp) a = do
        let (k, a') = f a
        case Map.lookup k mp of
                            ...
                            $ case res of
                                  Partial sres ->
                                      let ini1 = return (Partial sres)
                                          fld = Fold stp ini1 dn
                                          mp1 = Map.insert k fld mp
                                       in Partial $ Tuple' n mp1
                                  Done _ ->
                                      if n1 == 0
                                      then Done ()
                                      else let mp1 = Map.delete k mp
                                            in Partial $ Tuple' n1 mp1
                    -- XXX Maybe we should define this with "MonadError m =>"
                    Done _ -> error "Illegal state"
...
data ClassifyWithState s b m = ClassifyPartial !s !m | ClassifyDone !b !m

-- XXX This will run 1 unnecessary effect.
-- XXX Is there a way this can be designed properly
...
classifyWith f (Fold step initial extract) = Fold step' initial' extract'
    ...
    initial' = do
        ...
        return
            $ case res of
                  Partial fs -> Partial $ ClassifyPartial fs Map.empty
                  Done fb -> Partial $ ClassifyDone fb Map.empty

    step' (ClassifyPartial fs kv) a = do
        let k = f a
        case Map.lookup k kv of
            Nothing -> do
                res <- step fs a
                let kv1 = Map.insert k res kv
                resi <- initial
                case resi of
                    Partial fs1 -> return $ Partial $ ClassifyPartial fs1 kv1
                    Done fb -> return $ Partial $ ClassifyDone fb kv1
            Just x -> do
                case x of
                    Partial s -> do
                        r <- step s a
                        let kv1 = Map.insert k r kv
                        return $ Partial $ ClassifyPartial fs kv1
                    Done _ -> return $ Partial $ ClassifyPartial fs kv
    -- XXX Should we check for initial here?
    step' (ClassifyDone b kv) a = do
        let k = f a
        case Map.lookup k kv of
            Nothing -> do
                let kv1 = Map.insert k (Done b) kv
                 in return $ Partial $ ClassifyDone b kv1
            Just x -> do
                case x of
                    Partial _ ->
                        let kv1 = Map.insert k (Done b) kv
                         in return $ Partial $ ClassifyDone b kv1
                    Done _ -> return $ Partial $ ClassifyDone b kv
    ...
    extract' (ClassifyPartial _ kv) = extractify kv
    extract' (ClassifyDone _ kv) = extractify kv
