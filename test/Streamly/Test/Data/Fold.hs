...
find f ls = do
    ...
    case y of
        ...
        Just idx ->
            S.fold (FL.any f) (S.fromList $ take idx ls)
            `shouldReturn`
            False
...
all f ls =
    S.fold (FL.all f) (S.fromList ls)
    `shouldReturn`
    Prelude.and (map f ls)
...
mapM = forAll (listOf1 (chooseInt (intMin, intMax))) $ \ls ->
    monadicIO $ do
        v1 <-
            run
                $ S.fold
                    (FL.mapM (\x -> return (x+(Prelude.length ls))) FL.sum)
                $ S.fromList ls
